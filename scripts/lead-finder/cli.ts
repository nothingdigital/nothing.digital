...
async function main() {
  ...
  const scored = await scoreAndEnrich(candidates, {
  ...
  const filtered = scored.filter((lead) => lead.score >= minScore);
  ...
  console.log(
    `High-intent (score≥30): ${filtered.filter((l) => l.score >= 30).length}`,
  );
  console.log(
